...
class ElectronDownloader {
  ...
  downloadFile (url, cacheFilename, cb, onSuccess) {
    ...
    const gotOptions = {
      followRedirect: true,
      stream: true,
      agent: caw(this.proxy),
      rejectUnauthorized: this.strictSSL
    }
    const gotStream = got(url, gotOptions)
    gotStream.on('response', (response) => {
      if (response.statusCode < 200 || response.statusCode >= 400) {
        errored = true
        gotStream.destroy(new Error(`An unexpected status code was encountered while fetching "${url}": ${response.status}`))
      }
    })
    const handleError = (err) => {
      errored = true
      this.handleDownloadError(cb, err)
    }
    gotStream.on('error', handleError)
    const writeStream = fs.createWriteStream(path.resolve(this.cache, tempFileName))
    writeStream.on('error', handleError)
    gotStream.pipe(writeStream)

    writeStream.on('close', () => {
      if (!errored) this.moveFileToCache(tempFileName, cacheFilename, cb, onSuccess)
    })
  }
