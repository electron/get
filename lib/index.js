...
class ElectronDownloader {
  ...
  get arch () {
    let arch = this.opts.arch || process.arch
    if (arch === 'arm') {
      arch = `armv${process.config.variables.arm_version}${process.config.variables.icu_endianness}`
      if (arch === 'armv6l') arch = execSync('uname -m').toString().trim()
    }
